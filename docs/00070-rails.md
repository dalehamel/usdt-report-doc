...
A [sample app](https://github.com/dalehamel/usdt-rails-sample) is used to explain the usage of
tracers from `ruby-static-tracing` in Rails.

We can add `ruby-static-tracing` to our rails enabling tracers in our application config:
...
```bash
$ bpftrace -e 'usdt::sample_controller:* { @[str(arg0)] = lhist(arg1/1000000, 0, 2000, 100); }' -p 10954
Attaching 4 probes...
```
