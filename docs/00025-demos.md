...
This simplistic program will loop indefinitely, printing `Not Enabled` every second. This repre
ents the Ruby program going on it's merry way, doing what it's supposed to be doing - pretend t
at it is running actual application code. The application isn't spending any time executing pro
es, all it is doing is checking if the probe is enabled. Since the probe isn't enabled, it cont
nues with business as usual. The cost of checking if a probe is enabled is extraordinarily low
~5 micro seconds).
